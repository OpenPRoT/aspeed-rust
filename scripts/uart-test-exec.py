...
class UartTestExecutor:
    ...
    def __init__(self, args):
        ...
        self.log_file = args.log_file or f"uart-test-{os.getpid()}.log"
    ...
    def toggle_fwspick(self, state: str):
        ...
        # Note: FWSPICK uses 'pn dh' for high state as in original script
        if state == "dh":
            cmd_state = "pn dh"
        else:
            cmd_state = state
        ...
        try:
            ...
            raise RuntimeError(f"Failed to set FWSPICK {self.args.fwspick_pin} to {state}: {e}")
    ...
    def open_serial(self) -> bool:
        ...
        try:
            ...
            self.log(f"Serial port opened: {self.args.uart_device} @ {self.args.baudrate} baud")
    ...
    def upload_firmware(self) -> bool:
        ...
        try:
            ...
            # Upload firmware in chunks to avoid overwhelming the bootloader
            chunk_size = 1024
            bytes_sent = 0
    ...
    def monitor_test_execution(self, timeout: int = 600) -> bool:
        ...
        # Use command line timeout if provided, otherwise use parameter default
            ...
            if data:
                ...
                for line in lines:
                    ...
                    if 'panic' in line.lower():
                        self.log(f"\nPanic detected in test execution!")
                        return False
    ...
    def run_full_test_sequence(self) -> bool:
        ...
        try:
            ...
            # Monitor test execution (starts immediately after upload)
...
Examples:
  ...
  ./uart-test-exec.py --upload-only /dev/ttyUSB0 firmware.bin
    ...
    parser.add_argument("uart_device", nargs='?', 
                       help="UART device path (e.g., /dev/ttyUSB0)")
    parser.add_argument("firmware", nargs='?',
                       help="Firmware binary file path")
    
    # GPIO control
    parser.add_argument("--srst-pin", type=int, default=23,
                       help="SRST GPIO pin number (default: 23)")
    parser.add_argument("--fwspick-pin", type=int, default=18,
                       help="FWSPICK GPIO pin number (default: 18)")
    
    # Manual GPIO operations
    parser.add_argument("--manual-srst", choices=['low', 'high', 'dl', 'dh'],
                       help="Manually toggle SRST pin")
    parser.add_argument("--manual-fwspick", choices=['low', 'high', 'dl', 'dh'],
                       help="Manually toggle FWSPICK pin")
    
    # Sequence operations
    parser.add_argument("--sequence", choices=['fwspick-mode', 'normal-mode'],
